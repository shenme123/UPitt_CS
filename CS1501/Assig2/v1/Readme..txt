...
Stack is used for building the tree so when certain operater is met, the stack pops the operand from the top. For building the normalized tree, recursion is used. The normalize doesn't work right for some expession such as (!(!(!(!M)))).