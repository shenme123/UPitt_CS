    ...
    public CalcMatchingIndex(int [] tids, ArrayList<ArrayList> tagInfo, ArrayList<ArrayList> p_tagInfo) throws Exception{
        ...
        for (int i=0; i<3; i++){
            for (ArrayList pTag: p_tagInfo){
                if (Integer.parseInt(pTag.get(1).toString())==(tids[i])){
                    double f = Double.parseDouble( pTag.get(2).toString() );
        ...
        prop2score.add(Integer.parseInt( p_tagInfo.get(0).get(0).toString() ));
